...
public class EntityDreadMob extends EntityMob implements IDreadMob {
	...
	public boolean isOnSameTeam(Entity entityIn) {
		return entityIn instanceof IDreadMob
            || super.isOnSameTeam(entityIn)
            || IceAndFireCoreUtils.entityNameArrayContains(IceAndFire.CONFIG.dreadGang, entityIn);
	}
	...
	public void onLivingUpdate() {
		...
		if (!world.isRemote && this.getCommander() instanceof EntityDreadLich) {
			...
			EntityLivingBase lichAttackTarget = lich.getAttackTarget();
			if (
				lichAttackTarget != null
					&& lichAttackTarget.isEntityAlive()
					&& !IceAndFireCoreUtils.entityNameArrayContains(IceAndFire.CONFIG.dreadGang, lichAttackTarget)
			) {
				this.setAttackTarget(lichAttackTarget);
			}
		}
	}
