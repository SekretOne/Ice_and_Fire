...
public class EntityGorgon extends EntityMob implements IAnimatedEntity, IVillagerFear, IAnimalFear, IHumanoid {
	...
	public static boolean isEntityLookingAt(EntityLivingBase looker, EntityLivingBase seen, double degree) {
		...
		Vec3d vec3d1 = new Vec3d(seen.posX - looker.posX, seen.getEntityBoundingBox().minY + (double) seen.getEyeHeight() - (looker.posY + (double) looker
				.getEyeHeight()), seen.posZ - looker.posZ);
	...
	protected void initEntityAI() {
		...
		this.tasks.addTask(6, new EntityAIWatchClosest(this, EntityPlayer.class, 8.0F, 1.0F) {
			public boolean shouldContinueExecuting() {
				if (this.closestEntity != null && this.closestEntity instanceof EntityPlayer && ((EntityPlayer) this.closestEntity)
						.isCreative()) {
					return false;
				}
		...
		this.targetTasks.addTask(3, new EntityAINearestAttackableTarget(this, EntityLiving.class, 0, true, false, new Predicate<Entity>() {
			...
			public boolean apply(@Nullable Entity entity) {
				...
				return entity instanceof EntityLiving && DragonUtils.isAlive((EntityLiving) entity) && (properties == null || !properties.isStone) || !IsImmune
						.toStone(entity) && (entity instanceof IBlacklistedFromStatues && ((IBlacklistedFromStatues) entity)
						.canBeTurnedToStone());
			}
		}));
	...
	public boolean attackEntityAsMob(Entity entityIn) {
		boolean blindness = this.isPotionActive(MobEffects.BLINDNESS) || this.getAttackTarget() != null && this.getAttackTarget()
				.isPotionActive(MobEffects.BLINDNESS) || this.getAttackTarget() != null && IsImmune.toStone(this.getAttackTarget());
	...
	public void setAttackTarget(@Nullable EntityLivingBase living) {
		...
		if (living != null && !world.isRemote) {
			...
			boolean blindness = this.isPotionActive(MobEffects.BLINDNESS) || living.isPotionActive(MobEffects.BLINDNESS) || IsImmune
					.toStone(living) || ((living instanceof IBlacklistedFromStatues) && !((IBlacklistedFromStatues) living)
					.canBeTurnedToStone()) || isBlindfolded(living);
			if (blindness && this.deathTime == 0) {
	...
	protected void onDeathUpdate() {
		...
		if (this.world.isRemote) {
			for (int k = 0; k < 5; ++k) {
				...
				double d0 = 0.1;
				double d1 = 0.1;
				IceAndFire.PROXY.spawnParticle("blood", this.posX + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, this.posY, this.posZ + (double) (this.rand
						.nextFloat() * this.width * 2.0F) - (double) this.width, d2, d0, d1);
			}
		}
		if (this.deathTime >= 200) {
			if (!this.world.isRemote && (this.isPlayer() || this.recentlyHit > 0 && this.canDropLoot() && this.world.getGameRules()
			...
			for (int k = 0; k < 20; ++k) {
				...
				this.world.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, this.posX + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, this.posY + (double) (this.rand
						.nextFloat() * this.height), this.posZ + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, d2, d0, d1);
			}
		}
	}
	...
	public void onLivingUpdate() {
		...
		if (target != null) {
			boolean blindness = this.isPotionActive(MobEffects.BLINDNESS) || target.isPotionActive(MobEffects.BLINDNESS);
			this.getLookHelper()
					.setLookPosition(target.posX, target.posY + (double) target.getEyeHeight(), target.posZ, (float) this
							.getHorizontalFaceSpeed(), (float) this.getVerticalFaceSpeed());
			if (!blindness && this.deathTime == 0 && target instanceof EntityLiving && !(target instanceof EntityPlayer)) {
				forcePreyToLook((EntityLiving) target);
			}
		}
		
		if (target != null && isEntityLookingAt(this, target, 0.4) && isEntityLookingAt(target, this, 0.4) && !isBlindfolded(target)) {
			boolean blindness = this.isPotionActive(MobEffects.BLINDNESS) || target.isPotionActive(MobEffects.BLINDNESS) || IsImmune
					.toStone(target);
			if (!blindness && this.deathTime == 0) {
	...
	public void forcePreyToLook(EntityLiving mob) {
		mob.getLookHelper()
				.setLookPosition(this.posX, this.posY + (double) this.getEyeHeight(), this.posZ, (float) mob.getHorizontalFaceSpeed(), (float) mob
