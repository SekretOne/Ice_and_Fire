...
public class EntityHippogryph extends EntityTameable implements ISyncMount, IAnimatedEntity, IDragonFlute, IVillagerFear, IAnimalFear, IDropArmor, IFlyingMount {
	...
	protected void initEntityAI() {
		...
		this.targetTasks.addTask(5, new HippogryphAITarget<>(this, EntityLivingBase.class, false, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase && !(entity instanceof AbstractHorse) && DragonUtils
				.isAlive((EntityLivingBase) entity)));
	...
	private void initHippogryphInv() {
		...
		if (animalchest != null) {
			...
			if (world.isRemote) {
				...
				IceAndFire.NETWORK_WRAPPER.sendToServer(new MessageHippogryphArmor(this.getEntityId(), 2, this.getIntFromArmor(animalchest
						.getStackInSlot(2))));
			}
		}
	}
	...
	public int getIntFromArmor(ItemStack stack) {
		if (!stack.isEmpty()) {
			stack.getItem();
			if (stack.getItem() == IafItemRegistry.iron_hippogryph_armor) {
				return 1;
			}
		}
		if (!stack.isEmpty()) {
			stack.getItem();
			if (stack.getItem() == IafItemRegistry.gold_hippogryph_armor) {
				return 2;
			}
		}
		if (!stack.isEmpty()) {
			stack.getItem();
			if (stack.getItem() == IafItemRegistry.diamond_hippogryph_armor) {
				return 3;
			}
		}
		return 0;
	...
	public boolean processInteract(EntityPlayer player, EnumHand hand) {
		...
		boolean isDev = Objects.equals(s, "Alexthe666") || s.equals("Raptorfarian");
		if (this.isTamed() && this.isOwner(player)) {
			if (itemstack.getItem() == Items.DYE && itemstack.getMetadata() == 1 && this.getEnumVariant() != EnumHippogryphTypes.ALEX && isDev) {
				...
				for (int i = 0; i < 20; i++) {
					this.world.spawnParticle(EnumParticleTypes.REDSTONE, this.posX + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, this.posY + (double) (this.rand
							.nextFloat() * this.height), this.posZ + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, 0, 0, 0);
				}
				return true;
			}
			if (itemstack.getItem() == Items.DYE && itemstack.getMetadata() == 7 && this.getEnumVariant() != EnumHippogryphTypes.RAPTOR && isDev) {
				...
				this.playSound(SoundEvents.ENTITY_ZOMBIE_INFECT, 1, 1);
				for (int i = 0; i < 20; i++) {
					this.world.spawnParticle(EnumParticleTypes.CLOUD, this.posX + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, this.posY + (double) (this.rand
							.nextFloat() * this.height), this.posZ + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, 0, 0, 0);
				}
				return true;
			}
			if (itemstack.getItem() == Items.RABBIT_STEW && this.getGrowingAge() == 0 && !isInLove()) {
			...
			if (itemstack.getItem() == Items.STICK) {
				if (player.isSneaking()) {
					if (this.hasHomePosition) {
						...
						player.sendStatusMessage(new TextComponentTranslation("hippogryph.command.new_home", homePos.getX(), homePos
								.getY(), homePos.getZ()), true);
						return true;
					}
				} else {
			...
			if (itemstack.getItem() == Items.SPECKLED_MELON && this.getEnumVariant() != EnumHippogryphTypes.DODO) {
				...
				this.playSound(SoundEvents.ENTITY_ZOMBIE_INFECT, 1, 1);
				for (int i = 0; i < 20; i++) {
					this.world.spawnParticle(EnumParticleTypes.ENCHANTMENT_TABLE, this.posX + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, this.posY + (double) (this.rand
							.nextFloat() * this.height), this.posZ + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, 0, 0, 0);
				}
				return true;
			}
			if (itemstack.getItem() instanceof ItemFood && ((ItemFood) itemstack.getItem()).isWolfsFavoriteMeat() && this
					.getHealth() < this.getMaxHealth()) {
				this.heal(5);
				this.playSound(SoundEvents.ENTITY_GENERIC_EAT, 1, 1);
				for (int i = 0; i < 3; i++) {
					this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, this.posX + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, this.posY + (double) (this.rand
							.nextFloat() * this.height), this.posZ + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, 0, 0, 0, Item
		...
		if (hippogryphInventory != null) {
			...
			for (int i = 0; i < nbttaglist.tagCount(); ++i) {
				...
				if (world.isRemote) {
					...
					IceAndFire.NETWORK_WRAPPER.sendToServer(new MessageHippogryphArmor(this.getEntityId(), 1, chest.getItem() == Item
							.getItemFromBlock(Blocks.CHEST) && !chest.isEmpty() ? 1 : 0));
					IceAndFire.NETWORK_WRAPPER.sendToServer(new MessageHippogryphArmor(this.getEntityId(), 2, this.getIntFromArmor(hippogryphInventory
							.getStackInSlot(2))));
				}
			}
		}
		this.hasHomePosition = compound.getBoolean("HasHomePosition");
		if (hasHomePosition && compound.getInteger("HomeAreaX") != 0 && compound.getInteger("HomeAreaY") != 0 && compound
				.getInteger("HomeAreaZ") != 0) {
	...
	public boolean isRidingPlayer(EntityPlayer player) {
		return getRidingPlayer() != null && player != null && getRidingPlayer().getUniqueID()
	...
	public float getFlightSpeedModifier() {
		return IceAndFire.CONFIG.hippogryphFlightSpeed;
	}
	...
	public boolean attackEntityFrom(DamageSource dmg, float i) {
		if (this.isBeingRidden() && dmg.getTrueSource() != null && this.getControllingPassenger() != null && dmg.getTrueSource() == this
	...
	public void onLivingUpdate() {
		...
		if (this.getAnimation() == ANIMATION_BITE && this.getAttackTarget() != null && this.getAnimationTick() == 6) {
			...
			if (dist < 8) {
				this.getAttackTarget()
						.attackEntityFrom(DamageSource.causeMobDamage(this), ((int) this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE)
								.getAttributeValue()));
			}
		}
		...
		if (!world.isRemote && !this.isOverAir() && this.getNavigator()
				.noPath() && this.getAttackTarget() != null && this.getAttackTarget().posY - 3 > this.posY && this.getRNG()
				.nextInt(15) == 0 && this.canMove() && !this.isHovering() && !this.isFlying()) {
		...
		if ((flying || hovering) && ticksExisted % 20 == 0 && this.isOverAir()) {
			this.playSound(SoundEvents.ENTITY_ENDERDRAGON_FLAP, this.getSoundVolume() * (IceAndFire.CONFIG.dragonFlapNoiseDistance / 2), 0.6F + this.rand
					.nextFloat() * 0.6F * this.getSoundPitch());
		}
		...
		if ((properties == null || !properties.isStone) && (!world.isRemote && this.getRNG()
				.nextInt(FLIGHT_CHANCE_PER_TICK) == 0 && !this.isSitting() && !this.isFlying() && this.getPassengers()
				.isEmpty() && !this.isChild() && !this.isHovering() && !this.isSitting() && this.canMove() && !this.isOverAir() || this.posY < -1)) {
			this.setHovering(true);
			this.hoverTicks = 0;
			this.flyTicks = 0;
		}
		if (getAttackTarget() != null && !this.getPassengers()
				.isEmpty() && this.getOwner() != null && this.getPassengers().contains(this.getOwner())) {
			this.setAttackTarget(null);
		}
	}
	...
	public void onUpdate() {
		...
		if (this.attack() && this.getControllingPassenger() != null && this.getControllingPassenger() instanceof EntityPlayer) {
			...
			if (target != null && this.getAnimationTick() >= 10 && this.getAnimationTick() < 13) {
				target.attackEntityFrom(DamageSource.causeMobDamage(this), ((int) this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE)
						.getAttributeValue()));
			}
		}
		...
		if (this.isFlying() && !this.isHovering() && this.getControllingPassenger() != null && this.isOverAir() && Math.max(Math
				.abs(motionZ), Math.abs(motionX)) < 0.1F) {
			this.setHovering(true);
			this.setFlying(false);
		}
		if (this.isHovering() && !this.isFlying() && this.getControllingPassenger() != null && this.isOverAir() && Math.max(Math
				.abs(motionZ), Math.abs(motionX)) > 0.1F) {
			this.setFlying(true);
			this.setHovering(false);
		}
		...
		if (this.spacebarTicks > 10 && this.getOwner() != null && this.getPassengers()
				.contains(this.getOwner()) && !this.isFlying() && !this.isHovering()) {
			this.setHovering(true);
		}
		if (world.isRemote) {
			roll_buffer.calculateChainFlapBuffer(35, 8, 6, this);
		}
		if (this.getAttackTarget() != null && this.getRidingEntity() == null && this.getAttackTarget().isDead || this.getAttackTarget() != null && this
				.getAttackTarget() instanceof EntityDragonBase && ((EntityDragonBase) this.getAttackTarget()).isDead) {
			this.setAttackTarget(null);
		}
	}
	...
	public void flyTowardsTarget() {
		if (airTarget != null && isTargetInAir() && this.isFlying() && this.getDistanceSquared(new Vec3d(airTarget.getX(), this.posY, airTarget
				.getZ())) > 3) {
	...
	protected boolean isTargetInAir() {
		return airTarget != null && ((world.getBlockState(airTarget)
				.getMaterial() == Material.AIR) || world.getBlockState(airTarget).getMaterial() == Material.AIR);
	}
	...
	public void refreshInventory() {
		...
		if (this.world.isRemote) {
			...
			IceAndFire.NETWORK_WRAPPER.sendToServer(new MessageHippogryphArmor(this.getEntityId(), 1, chest.getItem() == Item
					.getItemFromBlock(Blocks.CHEST) && !chest.isEmpty() ? 1 : 0));
			IceAndFire.NETWORK_WRAPPER.sendToServer(new MessageHippogryphArmor(this.getEntityId(), 2, this.getIntFromArmor(this.hippogryphInventory
					.getStackInSlot(2))));
		}
	}
